...
class OrgHistoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'status' => 'required|boolean',
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:1024', // Validating image file
        ...
        if ($request->hasFile('image')) {
            ...
            $imagePath = $imagePath->storeAs('org/logos', $newFileName, 'public');
        ...
        if ($request->hasFile('document')) {
            $documentPath = $request->file('image');
            $originalName = pathinfo($documentPath->getClientOriginalName(), PATHINFO_FILENAME);
            $extension = $documentPath->getClientOriginalExtension();
            $timestamp = Carbon::now()->format('YmdHis');
            $newFileName = $timestamp . '_' . $originalName . '.' . $extension;

            $documentPath = $documentPath->storeAs('org/logos', $newFileName, 'public');
            // Store the image in the 'public/org_histories' directory and get the file path
            //$imagePath = $request->file('image')->store('org_histories', 'public');
        }
        ...
        $orgHistory->document = $documentPath; // Store the image path if available
